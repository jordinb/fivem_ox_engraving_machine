...
exports('engraving_machine', function(data, slot)
    ...
    local ok, message = lib.callback.await('ox_engraving_machine:prepare', false, machineSlot, targetSlot, engravingText)
    ...
    SetTimeout(Config.ClientUseTimeout or 15000, function()
        if busy and activeUseId == useId then
            busy = false
            TriggerServerEvent('ox_engraving_machine:cancel')
